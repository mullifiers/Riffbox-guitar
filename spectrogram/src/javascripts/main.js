...
// var app= angular.module('riffbox',[])
...
var io  = socketio('http://localhost:8008/chat/')
console.log(io)
io.on('connect', function(){
	console.log('oipugyftrsetdryugyiij --------------------------------')
})
io.on('started',function(a){
	console.log('kjkhkbkjbkjbkjbkjbkjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj');
	console.log(a);
})	
$('#process').click(function(){
	
        io.emit('start_process')
		window.location.replace('http://localhost:8008/status/')
})
// -~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
...
$(function(){
	...
	var startup = function (){
		...
		sp.play('bin/snd/empty.mp3')

		sp.loopChanged(true)
		$('.music-box__tool-tip').hide(0);
		
		$('#loadingMessage').text('Please Wait...while Loading');
		$('#loadingSound').show(3000);
		sp.startRender()
		...
		$('.music-box__buttons__button').click(function(e){
			...
			if($(this).hasClass('selected')) {
				...
				// Check for play audio data instruction **************************
					...
					uploadfile();
					sp.loopChanged( true );
					$('#loadingMessage').text('Please Wait... Loading');
					sp.stop();
				}
			}
		})
